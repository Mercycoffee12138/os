...
/*
有关page的结构体定义都在#include <memlayout.h>中
*/
...
default_init_memmap(struct Page *base, size_t n) {
    ...
    //在这里我们进行全局操作，首先将base的property变成n，表示这是一个n页的空闲块
    base->property = n;
    SetPageProperty(base);//表示这里是空闲块头
    nr_free += n;//全局空闲页数增加n
    //接下来就是对freelist的插入操作
    if (list_empty(&free_list)) {
        list_add(&free_list, &(base->page_link));//freelist为空则直接插入
    } else {//如果不为空则按地址升序插入（找到小于该地址的page，并插到他前面）
        list_entry_t* le = &free_list;
        while ((le = list_next(le)) != &free_list) {
...
default_free_pages(struct Page *base, size_t n) {
    ...
    for (; p != base + n; p ++) {
        /*
        PageReserved(p) 为 false，说明这个页不是内核专用、硬件映射、页表等特殊用途的页，而是普通的、可用的物理内存页。
        只有“没有被保留”的页才能参与内存分配和回收，被保留的页不能被用户或分配器使用。
        */
        assert(!PageReserved(p) && !PageProperty(p));//保证没有被保留且不是块头
        p->flags = 0;//标志位清空
        set_page_ref(p, 0);//清空引用计数，表示是空闲的
    }
    //和alloc_memmap相同的操作，只不过这次是把用过的页面来加入其中
    base->property = n;
